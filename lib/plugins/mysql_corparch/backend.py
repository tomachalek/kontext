...
class Backend(DatabaseBackend):
    ...
    @asynccontextmanager
    async def cursor_from_ctx(self, plugin_ctx, dictionary=True):
    ...
    async def load_corpus_as_source_info(
            self, cursor: MySQLCursorAbstract, corpus_id: str, user_lang: str) -> Optional[str]:
    ...
    async def load_corpora_descriptions(
            self, cursor: MySQLCursorAbstract, corp_ids: List[str], user_lang: str) -> Dict[str, str]:
        if len(corp_ids) == 0:
            return {}
    ...
    async def load_corpus(self, cursor: MySQLCursorAbstract, corp_id: str) -> Dict[str, Any]:
        await cursor.execute(
            ...
            'LEFT JOIN kontext_subcorpus AS ks ON ks.id = kps.id '
            'LEFT JOIN kontext_alt_corpus AS ac ON ac.corpus_name = c.name '
            'WHERE c.active = 1 AND c.name = %s '
            'GROUP BY c.name ', (corp_id,))
        return await cursor.fetchone()

    async def list_corpora(
            self, cursor: MySQLCursorAbstract, user_id, substrs=None, keywords=None, min_size=0, max_size=None,
            requestable=False, offset=0, limit=10000000000, favourites=()) -> Iterable[Dict[str, Any]]:
