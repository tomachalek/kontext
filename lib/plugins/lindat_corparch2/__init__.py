...
from plugin_types.corparch import (
    AbstractSearchableCorporaArchive, CorplistProvider)
...
class CorpusArchive(AbstractSearchableCorporaArchive):
    ...
    async def get_list(self, plugin_ctx: PluginCtx, user_allowed_corpora=None):
        ...
        for item in (await self._raw_list(plugin_ctx)).values():
            ...
            if user_allowed_corpora is None or corp_id in user_allowed_corpora:
                ...
                try:
                    corp_info = await plugin_ctx.corpus_factory.get_info(corp_id)
    ...
    async def _process_corpus_node(self, plugin_ctx: PluginCtx, node, path, data):
        ...
        if meta_elm is not None:
            ...
            ans.metadata.desc = (await plugin_ctx.corpus_factory.get_info(ans.id)).description
    ...
    async def _export_featured(self, plugin_ctx: PluginCtx):
        ...
        for x in list((await self._raw_list(plugin_ctx)).values()):
            if x.id in permitted_corpora and is_featured(x):
                corpus_info = await cm.get_info(x.id)
                featured.append({
    ...
    async def export_favorite(self, plugin_ctx: PluginCtx, favitems):
        ...
        for item in favitems:
            ...
            tmp['description'] = self._export_untranslated_label(
                plugin_ctx, (await plugin_ctx.corpus_factory.get_info(item.main_corpus_id)).description)
            ans.append(tmp)
        return ans
