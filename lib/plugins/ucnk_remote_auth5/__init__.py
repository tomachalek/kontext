...
class CentralAuth(AbstractRemoteAuth):
    ...
    async def revalidate(self, plugin_ctx):
        ...
        cookie_sid = plugin_ctx.cookies.get(self._conf.cookie_sid, '')
        cookie_at = plugin_ctx.cookies.get(self._conf.cookie_at, '')
        cookie_rmme = plugin_ctx.cookies.get(self._conf.cookie_rmme, '0')
        cookie_lang = plugin_ctx.cookies.get(self._conf.cookie_lang, 'en')
        api_args = [
            ('sid', cookie_sid),
            ('at', cookie_at),
            ('rmme', cookie_rmme),
    ...
    async def corpus_access(self, plugin_ctx, corpus_name: str) -> CorpusAccess:
        if corpus_name == IMPLICIT_CORPUS:
            return CorpusAccess(False, True, '')
        with self._db.cursor_from_ctx(plugin_ctx) as cursor:
